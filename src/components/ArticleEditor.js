...
export default function ArticleEditor({ article }) {
  ...
  const inputCls =
    'w-full border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:border-navy transition-colors'
  const labelCls =
    'block text-xs font-semibold tracking-widest uppercase text-gray-400 mb-1.5'
    ...
    <div className="max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="text-sm text-navy border border-navy px-4 py-2 hover:bg-navy hover:text-white transition-colors"
          >
      ...
      {preview ? (
        <div className="bg-white shadow p-8">
          ...
          <h2 className="font-serif text-4xl text-navy font-bold mt-2 mb-4">
            {form.title || 'Sans titre'}
          </h2>
          {form.excerpt && (
            <p className="text-gray-500 text-lg italic mb-6 pb-6 border-b">
              {form.excerpt}
            </p>
          )}
          <div
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="IA, Comores, Tech..."
                />
              </div>
              <div>
                <label className={labelCls}>Tags (virgule)</label>
                <input
                  className={inputCls}
                  value={form.tags}
                  onChange={(e) => set('tags', e.target.value)}
                  placeholder="BERT, NLP, Python"
                />
              </div>
            </div>
      ...
      <div className="flex gap-3 mt-6 items-center">
        ...
        <button
          onClick={() => router.back()}
          className="text-sm text-gray-400 hover:text-navy ml-2"
        >
          Annuler
        </button>
      </div>
